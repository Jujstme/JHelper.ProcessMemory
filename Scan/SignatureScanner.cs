...
internal static class SignatureScanner
{
    ...
    /// WARNING: This struct uses quite a bit of unsafe code and is NOT to be exposed outside its class,
    /// hence it's marked as private and MUST stay private.
    /// </summary>
    private struct ScanEnumerator
    {
        internal int Current { get; private set; }    // The current position in the buffer (index of the found pattern)
        ...
        /// <param name="noOfElements">The number of elements (bytes) to scan from the start index.</param>
        internal ScanEnumerator(ScanPattern signature, ReadOnlySpan<byte> data)
        {
            currentCursor = 0;
        ...
        internal unsafe bool MoveNext(ScanPattern signature, ReadOnlySpan<byte> data)
        {
