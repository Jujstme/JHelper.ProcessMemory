...
internal static partial class WinAPI
{
    ...
    [SkipLocalsInit]
    internal static bool ReadString(IntPtr processHandle, IntPtr address, int maxLength, StringType stringType, out string result, string defaultValue = "")
    {
        // Validate the length input
        if (maxLength < 0)
            throw new ArgumentException("String length cannot be lower than 0.");

        // If maxLength is zero, return an empty string
        if (maxLength == 0)
        {
            result = string.Empty;
            return true;
